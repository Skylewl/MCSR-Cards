...
def update_player_list():
    ...
    if response.status_code == 200:
        ...
        with open("C:/Users/skylewl/Desktop/prjcts/mcsr_cards_discordbot/player_list.json", "w") as file:
            json.dump(data, file, indent=4)

        with open("C:/Users/skylewl/Desktop/prjcts/mcsr_cards_discordbot/player_list.json", "r") as file:
...
def update_player_list_pbs():
    ...
    if response.status_code == 200:
        ...
        with open("C:/Users/skylewl/Desktop/prjcts/mcsr_cards_discordbot/player_list_pbs.json", "w") as file:
            json.dump(data, file, indent=4)

        with open("C:/Users/skylewl/Desktop/prjcts/mcsr_cards_discordbot/player_list_pbs.json", "r") as file:
            original_data = json.load(file)
        #     filtered_data = [{'uuid': item['uuid'], 'name': item['name']} for item in original_data]
...
def get_random_player_name():
     response = requests.get(f"https://playerdb.co/api/player/minecraft/{roll_player.random_player()}")
...
def stats_from_name(name):
    response = requests.get(f'https://paceman.gg/stats/api/getSessionStats?name={name}&hours=999999&hoursBetween=999999')
    data = response.json()
    print(data)
    return data
...
def create_card(name, player_stats, uuid=None):
    ...
    card = Card(player_name, player_uuid, player_image, player_pb, player_nea, player_fsa, player_ssa, player_fpa, player_sea, player_eea, player_fa)
    return card
